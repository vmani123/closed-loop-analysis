...
input_files = st.file_uploader('load your timestamp file', accept_multiple_files=True)
try:
    df = pd.read_csv(input_files[0])
    time_since = np.cumsum(np.diff(np.array(df)))
    col1, col2 = st.columns(2)
    start = col1.number_input('time start in seconds', min_value=0, max_value=int(time_since[-1]))
    stop = col2.number_input('time stop in seconds', min_value=0, max_value=int(time_since[-1]))
    if stop <= start:
